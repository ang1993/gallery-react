...
const SingleArtworkComponent = () => {
  const { singleArtwork, loadingSingleArtwork } = useSelector(
    ...
    <div className="myRow">
      <div className="myColArt">
        <div className="artworkImageContainer">
          ...
          {singleArtwork.images && singleArtwork.images.web ? (
            <img
              src={singleArtwork.images.web.url}
              width={"500"}
        ...
        <SimpleBar style={{ maxHeight: 400 }} autoHide={false}>
          ...
          {singleArtwork.technique ? (
            <p className="labelInfo">
              Technique:{" "}
              <span className="labelData">{singleArtwork.technique}</span>
            </p>
          ) : (
            ""
          )}
          {singleArtwork.measurements ? (
            <p className="labelInfo">
              Dimensions:{" "}
              <span className="labelData">{singleArtwork.measurements}</span>
            </p>
          ) : (
            ""
          )}
          ...
          {singleArtwork.creators?.map((artist) => (
            <div>
              {artist.biography ? (
                <p className="labelInfo">
                  Bio: <span className="normalP">{artist.biography}</span>
                </p>
              ) : (
                ""
              )}
          ...
          {singleArtwork.fun_fact ? (
            <p className="labelInfo">
              Fun fact:{" "}
              <span className="normalP">{singleArtwork.fun_fact}</span>
            </p>
          ) : (
            ""
          )}
